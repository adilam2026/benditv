...
function renderMapBlock(list) {
  const zones = {};
  list.forEach((p) => { (zones[p.zone] = zones[p.zone] || []).push(p); });
  let pinsHtml = "";
  Object.entries(zones).forEach(([zone, items]) => {
    const base = ZONE_MAP_POS[zone];
    if (STATE.mapExpandedZone === zone) {
      items.forEach((p) => {
        pinsHtml += `<button class="map-pin" style="left:${p.map.x}%;top:${p.map.y}%" data-action="show-pin" data-id="${p.id}">
          <span class="bubble">${p.rating.toFixed(1)}</span>
        </button>`;
      });
    } else {
      pinsHtml += `<button class="map-pin cluster" style="left:${base.x}%;top:${base.y}%" data-action="expand-zone" data-zone="${esc(zone)}">
        <span class="bubble">${zone} · ${items.length}</span>
      </button>`;
    }
  });
  return `
  <div class="map-wrap">
    <img class="hero-map" src="assets/hero-map.svg" alt="Carte stylisée de Casablanca (illustration)">
    <span class="map-zone-label">${icon("pin", "icon-sm")} Casablanca</span>
    <div class="map-user" style="left:48%;top:58%" title="Votre position"></div>
    ${pinsHtml}
...
function clearGroupFilter() { STATE.groupFilter = null; STATE.groupLabel = null; }
...
document.addEventListener("click", (e) => {
  ...
  switch (action) {
    ...
    case "reset-filters": STATE.activeFilters.clear(); STATE.removedChips.clear(); closeSheet(); route(); break;
    case "apply-filters": closeSheet(); route(); break;
    case "set-sort": STATE.sort = el.dataset.key; closeSheet(); route(); break;
    case "set-view-mode": STATE.viewMode = el.dataset.mode; route(); break;
    case "remove-chip": {
      const k = el.dataset.key;
    ...
    case "expand-zone": STATE.mapExpandedZone = el.dataset.zone; route(); break;
    case "recenter-map": STATE.mapExpandedZone = null; route(); break;
    case "around-me-map": showToast("Localisation simulée : centré sur votre position"); STATE.mapExpandedZone = null; route(); break;
